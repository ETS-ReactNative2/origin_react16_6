...
export function requestCurrentTimeForUpdate() {
  if ((executionContext & (RenderContext | CommitContext)) !== NoContext) {
    ...
    return msToExpirationTime(now());// max-performance.now()/10
  }
...
export function computeExpirationForFiber(
  ...
  const priorityLevel = getCurrentPriorityLevel();// 获取当前调度正在执行的任务的优先级
  if ((mode & ConcurrentMode) === NoMode) {//没有ConcurrentMode
  ...
  if ((executionContext & RenderContext) !== NoContext) {
    ...
    return renderExpirationTime;//NoWork
  }
...
export function scheduleUpdateOnFiber(
  ...
  checkForNestedUpdates();//检查嵌套更新上线五
...
function markUpdateTimeFromFiberToRoot(fiber, expirationTime) {
  ...
  let root = null;
  if (node === null && fiber.tag === HostRoot) {//当前Fiber是 RootFiber
